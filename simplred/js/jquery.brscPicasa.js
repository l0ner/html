...
(function($) {
	$.fn.brscPicasa = function(options) {
		...
		return this.each(function() {
			var elem = this;
			function _start() {
				if (!opts.mode || !opts.user || !methods[opts.mode]) { 
					// fallback to from_link method if mode is empty or user is not set or unknown mode is set
					opts.mode = 'from_link';
				}
				methods['from_link'].apply(elem, [opts]);
			...
			var methods = {
					...
					$.getJSON(
						...
						function(root) {
							var entry = root.entry || {},
								$div = $('<div class="' + name 
									+ 'SingleItem ' + o.classes.single_item 
									+ '"><img class="' + name + 'Photo ' + o.classes.photo 
				...
				from_link: function(o) {
					...
					$links.each(function() {
						...
						return methods['photo'].apply($target, [o]);
				...
				to_link: function(o) {
					...
					$links.each(function() {
						...
						} else if (path_split[0] == 'm' && path_split.length == 2) { 
							// link to mobile version of Picasa
							if (this.search) {
								var params = this.search.replace('?', '').split('&');
								for(var i = 0; i < params.length; i++) {
									param = params[i].split('=');
